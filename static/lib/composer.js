...
define('composer', [
	...
	composer.enhance = function (postContainer, post_uuid, postData) {
		...
		*/
		...
		var submitBtnAnon = postContainer.find('.composer-submit-anonymously');
		...
		submitBtnAnon.on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();	// Other click events bring composer back to active state which is undesired on submit

			$(this).attr('disabled', true);
			postAnonymously(post_uuid);
		});
	...
	async function postAnonymously(post_uuid) {
		...
		var thumbEl = postContainer.find('input#topic-thumb-url');
		var onComposeRoute = postData.hasOwnProperty('template') && postData.template.compose === true;
		const submitBtnAnon = postContainer.find('.composer-submit-anonymously');
		...
		api[method](route, composerData)
			.then((data) => {
				submitBtnAnon.removeAttr('disabled');
				postData.submitted = true;
